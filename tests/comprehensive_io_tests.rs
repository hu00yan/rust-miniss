...
use rust_miniss::{BufferPool, IoBackend, Op, IoToken, CompletionKind, IoError, DummyIoBackend};
...
mod buffer_pool_tests {
    ...
    fn test_buffer_pool_size_limit() {
        ...
        for _ in 0..110 { // POOL_SIZE is 100
...
mod io_future_tests {
    ...
    impl IoBackend for MockIoBackend {
        ...
        fn poll_complete(&self, _cx: &mut Context<'_>) -> Poll<Vec<Self::Completion>> {
            let mut completions = self.completions.lock().unwrap();
            let ready_completions: Vec<_> = completions.drain().map(|(token, result)| {
                let op = Op::Read { fd: 1, offset: 0, len: 1024 }; // Dummy op
                (token, op, result)
            }).collect();
    ...
    async fn test_io_future_successful_completion() {
        ...
        // Submit an operation
        let op = Op::Read { fd: 1, offset: 0, len: 512 };
        ...
        tokio::spawn(async move {
            ...
            backend_clone.complete_operation(token, Ok(CompletionKind::Read {
                bytes_read: 512,
                data: vec![0u8; 512],
            }));
        });
...
mod failure_injection_tests {
    ...
    fn test_error_propagation_with_invalid_fd() {
        ...
        let read_result = unsafe {
            libc::read(invalid_fd, buffer.as_mut_ptr() as *mut libc::c_void, buffer.len())
        };
        assert!(read_result < 0, "Read from invalid fd should fail");
        
        let write_result = unsafe {
            libc::write(invalid_fd, b"This should fail".as_ptr() as *const libc::c_void, 15)
        };
    ...
    fn test_dummy_backend_error_propagation() {
        ...
        let op = Op::Read { fd: -1, offset: 0, len: 1024 }; // Invalid fd
        let token = backend.submit(op);
