...
impl IoUringBackend {
    ...
    pub fn register_buffer(&mut self, buffer: Vec<u8>) -> u64 {
    ...
    pub fn register_files(&mut self, files: &[RawFd]) -> Result<(), IoError> {
        self.ring.submitter()
    ...
    fn submit_read_op(&mut self, fd: RawFd, offset: u64, len: usize, token: IoToken) -> Result<(), IoError> {
        ...
        unsafe {
            match self.ring.submission().push(&read_entry) {
                Ok(_) => {
                    self.pending_ops.insert(token.id(), PendingOperation {
